...
export async function POST(request: Request) {
  try {
    ...
    // Format the data for GoHighLevel - adjusted for get started form structure
    const ghlData = {
      email: formData.email,
      phone: formData.phone,
      firstName: formData.firstName || (formData.name ? formData.name.split(' ')[0] : ''),
      lastName: formData.lastName || (formData.name && formData.name.includes(' ') ? formData.name.split(' ').slice(1).join(' ') : ''),
      customField: {
        // Add any specific fields from the get started form
      ...
      tags: ["website-lead", "renovation-bridge", "get-started"]
    };
